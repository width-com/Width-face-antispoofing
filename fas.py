...
class flip_mcl(nn.Module):
  ...
  def __init__(self, in_dim, ssl_mlp_dim, ssl_emb_dim, device=None):
  ...
  def forward_eval(self, input, norm_flag=True):
    ...
    for classname in (['spoof', 'real']):
        ...
        texts = clip.tokenize(texts).to(input.device) #tokenize
        class_embeddings = self.model.encode_text(texts) #embed with text encoder
        class_embedding = class_embeddings.mean(dim=0)
        ensemble_weights.append(class_embedding)
    text_features = torch.stack(ensemble_weights, dim=0).to(input.device)
